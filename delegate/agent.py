...
def build_system_prompt(
    ...
    # Researchers with a bound assistant get a short dynamic section that
    # names the partner and states the rules unique to the binding.  The
    # cognitive division, delegation protocol, and log-reading discipline
    # are all in the static researcher.md charter above — this section
    # only adds what depends on knowing the specific partner name.
    if role == "researcher":
        helper_name = state.get("assistant")
        if helper_name:
            role_parts.append(
                f"### Your Assistant — {helper_name}\n\n"
                f"Your assistant is **{helper_name}** (Haiku).  Apply the "
                f"Pipeline Discipline, Turn Discipline, and Token Efficiency "
                f"sections above by treating {helper_name} as your sidekick "
                f"for every mechanical task — experiment submission, log "
                f"reading, metric extraction, artifact saving, and draft "
                f"`knowledge_base.md` updates.\n\n"
                f"**Rules specific to your binding:**\n\n"
                f"- **Always include `task_id`** when messaging {helper_name} "
                f"— that's how they resolve which worktree to run in.\n"
                f"- **{helper_name} is yours alone.** No other agent on the "
                f"team can mail them, and they cannot mail anyone else.  "
                f"Do not relay third-party messages through them.\n"
                f"- **Ask 'what should I try next?'** — {helper_name} will "
                f"scan `experiment_summary_fail.md` → `direction.md` → "
                f"`brainstorm.md` and surface 2–3 candidates with IDs.  "
                f"They never recommend; you decide."
            )
