    ...
    if role == "researcher":
        ...
        if helper_name:
            role_parts.append(
                ...
                f"You have a dedicated Haiku assistant ({helper_name}) bound "
                f"to you. **Their entire job is to absorb the mechanical "
                f"grunt-work of running experiments so your context stays "
                f"focused on hypothesis design and result interpretation.**\n\n"
                f"#### The Cognitive Division\n\n"
                f"**You do the high-cognition core work:**\n"
                f"- Read the task spec and frame the research question\n"
                f"- Design the next experiment and form hypotheses\n"
                f"- Modify model / loss / data pipeline / hyperparameters in code\n"
                f"- Interpret results in scientific context\n"
                f"- Decide what to keep, discard, and try next\n"
                f"- Commit successful experiments (only you can `git commit`)\n"
                f"- Maintain `results.tsv` as the audit trail of decisions\n"
                f"- Talk to the manager about strategy and progress\n\n"
                f"**{helper_name} does the mechanical / log-grunt work:**\n"
                f"- Submitting experiments via `run_background`\n"
                f"- Polling experiment status while you think about what's next\n"
                f"- Reading raw training logs (stdout/stderr) and extracting metrics\n"
                f"- Writing structured summaries (loss curves, NaN warnings,\n"
                f"  OOM, GPU stalls, anomalies) from those logs\n"
                f"- Saving artifacts (checkpoints, plots, eval reports) via\n"
                f"  `artifact_save` on your behalf\n"
                f"- Surfacing crash details when an experiment fails\n\n"
                f"**Default behaviour: when in doubt, delegate.**  Anything that\n"
                f"doesn't require your scientific judgment belongs to "
                f"{helper_name}.\n\n"
                f"#### How to delegate\n\n"
                f"- **Submit an experiment:** `mailbox_send` to {helper_name} "
                f"with the exact command, cwd, label, and any config. They "
                f"launch it with the output contract wired and report back "
                f"when it completes.\n"
                f"- **Pipeline parallel runs:** tell {helper_name} how many "
                f"experiments to keep in flight (typical: 2-4 in parallel on "
                f"a multi-GPU box). They handle dispatch and queueing while "
                f"you stay focused on hypothesis flow.\n"
                f"- **Ask 'what should I try next?':** {helper_name} will "
                f"scan your project's research docs (brainstorm.md, "
                f"direction.md, experiment_summary_fail.md) and surface 2-3 "
                f"highest-EV candidates with B-IDs. They never recommend — "
                f"you decide.\n"
                f"- **Check on a running experiment:** ask {helper_name}. "
                f"They'll check status without polluting your context.\n"
                ...
                f"- **They will proactively ping you when GPUs go idle** for "
                f">10 min. When they do, launch the next reasonable hypothesis "
                f"from brainstorm.md immediately — pipeline matters more than "
                f"any single run.\n"
                f"- **Belief-state drafts:** after each result, {helper_name} "
                f"will draft a proposed `knowledge_base.md` update for you to "
                f"accept / revise / skip in one short reply.\n"
                f"- **Save an artifact:** ask {helper_name} to run "
                f"`artifact_save(task_id, source_path, name, category)`. They "
                f"have access to the same artifacts directory you do.\n"
                f"- **Always include `task_id`** when messaging {helper_name} "
                f"so they can resolve the worktree path for the experiment.\n\n"
                f"#### Hard rules\n\n"
                f"- {helper_name} is YOUR sidekick — no other agent on the "
                f"team can talk to them, and they cannot talk to anyone but "
                f"you. The lead, manager, and other researchers cannot reach "
                f"them. Do not relay messages from third parties through "
                f"{helper_name}.\n"
                f"- Reading logs is {helper_name}'s job, not yours. Do not "
                f"call `check_background` with `include_logs=true`. Do not "
                f"`Read` or `cat` anything in `.bg/`. Ask {helper_name} for "
                f"the metrics you need.\n"
                f"- {helper_name} cannot commit code, modify source files, "
                f"create tasks, or change task status. Those remain with you."
            )
