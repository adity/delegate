...
5. If approved: task moves to `in_approval`. Human gives final approval, or a `reviewer` agent auto-approves via `task_approve`/`task_reject` MCP tools, or auto-merge for auto-approval repos.
