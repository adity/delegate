...
4. Human approves (manual) or auto-approval (via reviewer agent or auto repos). If a `reviewer` agent is on the team and auto-approve is enabled, the daemon dispatches a review request. The reviewer calls `task_diff` to inspect the diff, checks for sensitive files and rebase status, then calls `task_approve` or `task_reject`. Sensitive files are escalated to the human.
